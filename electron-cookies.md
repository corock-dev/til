...
* `filter` Object 
  * `url` String (optional) -  `url` 과 관련있는 쿠키들을 회수합니다. 모든 URL의 쿠키를 회수하고 나면 Empty 상태를 나타냅니다.
...
쿠키 객체 배열로 resolve되는 프로미스 - `Promise<Cookie[]>` 를 반환합니다.
...
* `details` Object 
  * `url` String - 쿠키와 관련 있는 URL입니다. URL이 유효하지 않을 경우 프로미스는 rejected를 보냅니다.
